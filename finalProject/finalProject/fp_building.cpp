...
int main(void)
{
    ...
    // Initialize buildings with random positions and scales
    /*for (int i = 0; i < numBuildings; ++i) {
        float height = 1.0f + static_cast<float>(rand() % 10 + 1); // Random height between 1 and 10
        float xPos = static_cast<float>(rand() % 40 - 20); // Random x position between -50 and 50
        float zPos = static_cast<float>(rand() % 40 - 20); // Random z position between -50 and 50
        buildings[i].initialize(glm::vec3(xPos, 0.0f, zPos), glm::vec3(1.0f, height, 1.0f));
    }*/
    for (int i = 0; i < numBuildings; ++i) {
        float height = 1.0f + static_cast<float>(rand() % 10 + 1); // Random height between 1 and 10
        float xPos = static_cast<float>(rand() % 40 - 20); // Random x position between -20 and 20
        float zPos = static_cast<float>(rand() % 40 - 20); // Random z position between -20 and 20
        buildings[i].initialize(glm::vec3(xPos, 0.0f, zPos), glm::vec3(1.0f, height, 1.0f));
        buildings[i].setTexture(textures[i % textures.size()]);
    }
    // Camera setup
    lookat = glm::vec3(0.0f, 5.0f, 0.0f); // Adjust lookat dynamically
    viewAzimuth = glm::radians(45.0f);     // Angle in the XZ-plane
    viewPolar = glm::radians(30.0f);       // Elevation angle
    viewDistance = 120.0f;                 // Adjust the distance to get a good view of all buildings
